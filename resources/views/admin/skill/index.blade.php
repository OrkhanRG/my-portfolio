                        ...
                        <tr id="row-{{ $skill->id }}">
                            <td width="10">
                                <div class="media">
                                    <div class="avatar me-2">
                                        <img alt="avatar-{{ $skill->name }}" src="{{ asset($skill->image) }}" class="rounded-circle">
                                    </div>
                                </div>
                            </td>
                            <td>{{ $skill->name }}</td>
                            ...
                            <td class="text-center">
                                <div class="action-btns">
                                    <a href="{{ route('admin.skill.edit', $skill->id) }}" class="action-btn btn-edit bs-tooltip me-2"
                                    ...
                                    <a href="javascript:void(0);" data-id="{{ $skill->id }}" data-name="{{ $skill->name }}" class="action-btn btn-delete bs-tooltip"
    ...
    <script>
        $('document').ready(() => {
            $('.btn-change-status').on('click', function () {
                ...
                $.ajax({
                    url: "{{ route('admin.skill.change-status') }}",
                    type: 'POST',
                    data: {
                        id: id
                    },
            ...
            $('.btn-delete').on('click', function () {
                ...
                let route = "{{ route('admin.skill.destroy', 'test') }}"
                route = route.replace('test', id);

                Swal.fire({
                    title: name,
                    text:  "Bacarığını silmək istədiyinizə əminsiz?",
                    icon: 'warning',
